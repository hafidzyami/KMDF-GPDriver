...
BOOLEAN IsSafeProcessForRemoteThreads(PWCHAR ProcessImagePath, HANDLE TargetProcessId)
{
    ...
    if (wcsstr(lowerPath, L"\\windows\\system32\\") != NULL ||
        wcsstr(lowerPath, L"\\windows\\syswow64\\") != NULL)
    {
        return TRUE; // Core Windows system directories are safe
    }

    // Some paths in Program Files are typically safe
    if ((wcsstr(lowerPath, L"\\program files\\windows defender\\") != NULL) ||
        (wcsstr(lowerPath, L"\\program files\\microsoft office\\") != NULL) ||
        (wcsstr(lowerPath, L"\\program files (x86)\\microsoft office\\") != NULL))
    {
    ...
    if (wcsstr(lowerPath, L"\\temp\\") != NULL ||
        ...
        wcsstr(lowerPath, L".vbs") != NULL ||    // VBScript
        wcsstr(lowerPath, L"\\recycle") != NULL) // Recycle bin
    {
...
*/
VOID ThreadCreateNotifyWorkItemRoutine(
    _In_ PVOID Context)
{
        ...
        for (ULONG64 i = 0; i < ImageFilter::ProcessHistorySize && threadsCollected < MaxEntries; i++)
        {
            ...
            if (processEntry->ThreadHistorySize > 0 && processEntry->ThreadHistory != NULL)
            {
                ...
                for (ULONG j = 0; j < processEntry->ThreadHistorySize && threadsCollected < MaxEntries; j++)
                {
                    ...
                    if ((j % 3) == 0)
                    { // Simulating suspended state for demo
